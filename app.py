...
<style>
    ...
    .reason-title { font-size: 12px; font-weight: 700; color: #1e3a5f; margin-bottom: 6px; letter-spacing: 0.5px; }
...
def generate_reasons(feats, hb_type, preds, real_bp=None):
    ...
    if hb_type == "Strong":
        ...
        reasons.append(("🔴 분류 근거 — Strong HB",
            f"TPSA={feats['TPSA']}Å² (높음), HBD={feats['HBD']}{', '+oh_str if oh_str else ''} → "
    ...
    if preds and real_bp:
        ...
        hb_note = (
            ...
            if feats['NumOH'] >= 2
            else "무극성 분자는 MW·LogP가 선형적으로 끓는점 결정 → Ridge도 안정"
            if hb_type == "NonPolar"
            else "쌍극자 특성이 분자마다 달라 트리 모델이 유리한 경향"
        )
...
def mol_to_svg(mol):
    if not DRAW_AVAILABLE or mol is None:
        return None
    try:
        mol_2d = Chem.MolFromSmiles(Chem.MolToSmiles(mol))
...
with st.sidebar:
    ...
    st.markdown("---")
    st.markdown("### 입력 방법")
    input_mode = st.radio("", ["분자 이름 선택", "SMILES 직접 입력"],
...
with st.spinner("모델 초기화 중... (첫 실행 시 30초 소요)"):
...
# ── 예측 실행 ────────────────────────────────────────────────
if predict_btn or input_mode == "분자 이름 선택":
    ...
    with tab1:
        ...
        with c1:
            ...
            fig_bar.update_layout(
                ...
                showlegend=False,
                title=dict(text="5종 모델 끓는점 예측", font=dict(size=14,color='#1e3a5f'))
            )
        ...
        if real_bp:
            ...
            fig_e.update_layout(
                ...
                showlegend=False,
                title=dict(text="모델별 예측 오차", font=dict(size=13,color='#1e3a5f'))
            )
    ...
    with tab3:
        ...
        with c4:
            ...
            fig_shap.update_layout(
                ...
                xaxis=dict(gridcolor='#f1f5f9'),
                showlegend=False,
                title=dict(text="SHAP 특성 중요도 (RF)", font=dict(size=13,color='#1e3a5f'))
            )
